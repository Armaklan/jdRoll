...
DROP PROCEDURE IF EXISTS `FDP_update`;$$
CREATE PROCEDURE FDP_update()
BEGIN
    
    IF VERSION_EXISTS(2) = 0 THEN
       
	   ---
	   --- Structure de la table fiche de personnage
	   ---
	   
		CREATE TABLE IF NOT EXISTS `SHEET_FIELD_TEMPLATE` (
          `id` int(11) NOT NULL AUTO_INCREMENT,
		  `campagne_id` int(10) unsigned NOT NULL,
          `name` varchar(500) NOT NULL,
		  `type` int(2) NOT NULL DEFAULT 0,
		  `font_color` varchar(10) DEFAULT NULL,
		  KEY `IDX_SHEET_FIELD_TMPL_CAMP` (`campagne_id`),
          PRIMARY KEY (`id`)
        ) ENGINE=MyISAM DEFAULT CHARSET=latin1;
		
		ALTER TABLE `SHEET_FIELD_TEMPLATE`
			ADD CONSTRAINT `FK_SHEET_FIELD_TMPL_CAMP` FOREIGN KEY (`campagne_id`) REFERENCES `campagne_config` (`campagne_id`) ON DELETE CASCADE;
